...
//------------------
void BinaCPP::get_exchangeInfo(Document &json_result)
{
	...
	if ( str_result.size() > 0 )
	{
		...
		try
		{
			json_result.Parse(str_result.c_str());
		}
		catch (exception &e)
		{
...
//GET /api/v1/time
//------------
void BinaCPP::get_serverTime(Document &json_result)
{
...
*/
void BinaCPP::get_allPrices(Document &json_result)
{
...
BinaCPP::get_price(const char *symbol)
{
	...
	if (alltickers.IsArray())
	{
		for (int i = 0; i < alltickers.Size(); i++)
		{
			if (alltickers[i]["symbol"].GetString() == str_symbol)
			{
				ret = atof(alltickers[i]["price"].GetString());
				break;
			}
		}
	}
...
void BinaCPP::get_allBookTickers(Document &json_result)
{
...
//--------------
void BinaCPP::get_bookTicker(const char *symbol, Value &json_result)
{
...
symbol	STRING	YES	
*/
void BinaCPP::get_24hr(const char *symbol, Document &json_result)
{
...
void BinaCPP::get_account(long recvWindow, Document &json_result)
{
...
void BinaCPP::start_userDataStream(Document &json_result)
{
...
//--------------------------------------------------
void BinaCPP::curl_api(string &url, string &result_json)
{
	vector<string> v;
	string action = "GET";
	string post_data = "";
...
//--------------------
// Do the curl
void BinaCPP::curl_api_with_header(string &url, string &str_result, vector<string> &extra_http_header, string &post_data, string &action)
{
